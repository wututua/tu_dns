...
> **Related docs**: [部署与运维](deploy.md)、[安全](security.md)
...
数据库配置与备份策略详见 [部署与运维](deploy.md)。迁移由 GORM AutoMigrate 在启动时完成，当前没有版本化 SQL migration。

## Security and Reliability

数据库安全基线详见 [安全](security.md) 和 [部署与运维](deploy.md)。在存储层面，确保数据库文件与 DSN 只对服务账号可读，远程数据库使用私网/TLS，且不在日志或 Issue 中暴露 DSN。
